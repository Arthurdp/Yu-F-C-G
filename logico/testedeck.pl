...
deckJogador(Jogador, Deck):- Jogador = [_,_,Deck,_,_,_,_,_].

colecaoJogador(Jogador, Colecao):- Jogador = [_,Colecao,_,_,_,_,_,_].
...
removeAdd(Jogador,Carta, R):- Jogador = [_, Colecao, Deck, _,_,_,_,_], member(Carta, Deck),
    add(Colecao, Carta, NovaColecao), remove(Deck, Carta, NovoDeck),
    R = [_, NovaColecao, NovoDeck, _,_,_,_,_], nl,writeln('Carta removida do deck!'), nl.
...
gerenciaDeck(3, Jogador):- Jogador = [_, _, Deck, _,_,_,_,_], length(Deck, Len), Len < 20, nl, 
...
addRemove(Jogador,Carta, R):- Jogador = [_, Colecao, Deck, _,_,_,_,_], member(Carta, Colecao),
    add(Deck, Carta, NovoDeck), remove(Colecao, Carta, NovaColecao),
    R = [_, NovaColecao, NovoDeck, _,_,_,_,_], nl, writeln('Carta adicionada ao deck!'), nl.

addRemove(Jogador,_, Jogador):-nl,  writeln('Você não possui esta carta.'), nl. 

gerenciaColecao(1, Jogador):- Jogador = [_, _, Deck, _,_,_,_,_], length(Deck, Len), Len =:= 20,nl,
...
jog(Jogador) :- Jogador = ['a',[],[1, 2],8000,[],[],[],[]].
